                ...
                $users = $query->select('users.*')
                    ...
                    ->map(function ($coach) {
                        ...
                        $isOnline = $coach->is_online ||
                            ($coach->last_seen_at && $coach->last_seen_at->gt(now()->subMinutes(5)));
                        ...
                        return [
                            ...
                            'unread_count' => $coach->unread_count,
                            'user_type' => $coach->user_type,
                            'is_online' => $isOnline, // Add online status
                            'last_seen_at' => $coach->last_seen_at ? $coach->last_seen_at->toISOString() : null, // Add last seen
                        ];
                    });
                ...
                $users = $query->select('users.*')
                    ...
                    ->map(function ($student) {
                        ...
                        $isOnline = $student->is_online ||
                            ($student->last_seen_at && $student->last_seen_at->gt(now()->subMinutes(5)));
                        ...
                        return [
                            ...
                            'unread_count' => $student->unread_count,
                            'is_online' => $isOnline, // Add online status
                            'last_seen_at' => $student->last_seen_at ? $student->last_seen_at->toISOString() : null, // Add last seen
                        ];
                    });
