                            ...
                            <div class="container-fluid chat-box-add user-admin-chatbox">
                                @if($chatList->isEmpty())
                                    <div class="no-chat-message text-center p-4 w-100">
                                        <p class="text-danger"><i class="bi bi-inbox"></i>No Chat Fount </p>
                                    </div>
                                @else
                                ...
                                <div class="row no-gutters chat-inner-user">      
                                    ...
                                    <div class="col-md-3 border-right left-side-chat">                            
                                        ...
                                        @foreach($chatList as $person)
                                            ...
                                            <div class="friend-drawer friend-drawer--onhover vendor-item"
                                                ...
                                                data-image="{{ $person->profile_image ? asset('public/uploads/profile_image/'.$person->profile_image) : url('public/img/user.png') }}" data-unread="{{ $person->unread_count }}">
                                    ...
                                    <div class="col-md-9 right-side-chat-add d-none" id="chatPanelWrapper">                        
                                        ...
                                        <div class="chat-panel message-area" id=""></div>                                
                                        <div class="chat-box-tray type-section">
                                            <input type="text" id="messageInput" placeholder="write your message" />

                                            <!-- <label for="attachment" style="cursor: pointer;"> <i class="bi bi-paperclip"></i>
                                                <input type="file" id="attachment" accept=".jpg,.jpeg,.png,.pdf" style="display: none;">
                                            </label> -->

                                            <div id="attachmentPreview" style="margin-top: 10px; display: none; position: relative;">
                                                <span id="removeAttachment" style="position: absolute; top: -10px; right: -10px; cursor: pointer; font-size: 20px; color: red;">&times;</span>
                                    ...
                                    <div class="col-md-9" id="noChatSelected" style="height:520px;">
                                        <div class="d-flex justify-content-center align-items-center h-100 flex-column text-center p-5">
                                            ...
                                            <h5 class="mt-3 text-muted">select a conversion to start chat</h5>
                                        </div>
                                    </div>
     ...
     <script>
        ...
        function updateUnreadCount(e) {
                ...
                if (isActive) {
                    ...
                    // alert('New message from ' + e.message.senderId + ': ' + e.message.message);
        ...
        $('#sendMessageBtn').on('click', sendMessage);
          ...
          $(document).on('keydown', function (e) {
                if (e.key === 'Enter' && !e.shiftKey) {
                    const message = $('#messageInput').val().trim();
                    if (message.length > 0 || isFileSelected) {
                        e.preventDefault(); 
                        sendMessage();
                    }
                }
        ...
        $('#attachment').on('change', function () {
            ...
            const previewBox = $('#attachmentPreview');
           ...
           isFileSelected = true;
            ...
            if (fileType.startsWith('image/')) {
                ...
                reader.onload = function (e) {
                    previewContent.html(`<img src="${e.target.result}" style="max-height: 80px;" />`);
                    previewBox.show();
                };
                reader.readAsDataURL(file);
            } else if (fileType === 'application/pdf') {
                previewContent.html(`<a href="#" style="color: blue; font-weight: bold;" onclick="return false;">📄 ${file.name}</a>`);
                previewBox.show();
            } else {
                previewContent.html('');
                previewBox.hide();
